...
import { Heart, User, Lock, Mail, ArrowRight, ShieldCheck, Loader2 } from 'lucide-react';
import FloatingHearts from '@/components/FloatingHearts';

export default function SignupPage() {
  const router = useRouter();
  const { user, firebaseUser, loading: authLoading, loginWithGoogle, signUpWithEmail } = useAuth();
  ...
  useEffect(() => {
    if (!authLoading && (user?.username || firebaseUser)) {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      await signUpWithEmail(identifier, password, name.trim());
    } catch (err: any) {
      ...
      <div className="w-full max-w-md space-y-6 relative z-10">
        ...
        <div className="text-center space-y-3">
          ...
          <div className="space-y-1">
            <h2 className="text-xl font-black text-white">Create an Account</h2>
            <p className="text-xs text-pink-200/70">
              Join CupidX in seconds and connect with people worldwide
            </p>
          </div>
        </div>
